...
function ChatWindow({ otherNickname, myNickname, onBack }: {
  ...
  useEffect(() => {
    fetch(`/api/messages/direct?with=${encodeURIComponent(otherNickname)}`)
      .then(r => r.json())
  ...
  const sendMessage = async () => {
    ...
    const res = await fetch('/api/messages/direct', {
      ...
      body: JSON.stringify({ to_nickname: otherNickname, content })
    })
    if (res.ok) {
          ...
          messages.map((m: any, i: number) => (
            <div key={m.id || i} className={`flex ${m.is_mine ? 'justify-end' : 'justify-start'}`}>
              <div className={`max-w-xs lg:max-w-sm px-4 py-2.5 rounded-2xl text-sm ${
                ...
                <p>{m.content}</p>
                <p className={`text-xs mt-1 ${m.is_mine ? 'text-blue-200' : 'text-gray-500'}`}>
    ...
    fetch('/api/auth/me')
      ...
      .then(async data => {
        if (data.user?.faceit_nickname) {
          ...
          // Загружаем список уникальных собеседников
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-gray-950 to-gray-900 text-white flex">
      ...
      <main className="flex-1 flex min-h-screen">
        {/* Список диалогов */}
        <div className={`${selectedNickname ? 'hidden md:flex' : 'flex'} flex-col w-full md:w-80 border-r border-gray-800/50 bg-gray-950/30`}>
          ...
          <div className="flex-1 overflow-y-auto">
            {loading ? (
              ...
              filtered.map(conv => (
                <button
                  key={conv.nickname}
                  onClick={() => setSelectedNickname(conv.nickname)}
                  className={`w-full flex items-center gap-3 px-4 py-4 border-b border-gray-800/30 text-left transition-all hover:bg-gray-800/30 ${selectedNickname === conv.nickname ? 'bg-gray-800/50' : ''}`}
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="font-semibold text-sm">{conv.nickname}</p>
                    <p className="text-xs text-gray-500 truncate">{conv.last_message}</p>
                  </div>
                  <p className="text-xs text-gray-600 flex-shrink-0">
                    {conv.last_at ? new Date(conv.last_at).toLocaleDateString('ru-RU', { day: 'numeric', month: 'short' }) : ''}
                  </p>
                </button>
              ))
            )}
        ...
        {/* Окно чата */}
        <div className={`${selectedNickname ? 'flex' : 'hidden md:flex'} flex-1 flex-col`}>
          {selectedNickname ? (
            <ChatWindow
              otherNickname={selectedNickname}
              myNickname={myNickname}
              onBack={() => setSelectedNickname(null)}
            />
          ) : (
            <div className="flex flex-col items-center justify-center h-full text-center py-12">
