...
export async function GET(request: Request) {
  ...
  try {
    ...
    const { data: users } = await supabase.from('users').select('id, email, faceit_nickname').eq('id', payload.userId).limit(1)
    if (!users || users.length === 0) return NextResponse.json({ user: null }, { status: 401 })
