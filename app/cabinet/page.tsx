...
function CabinetContent() {
  ...
  // Реальный анализ демок через локальный demoparser2
  const handleFileParse = async (e: React.ChangeEvent<HTMLInputElement>) => {
    const file = e.target.files?.[0]
    if (!file || !nickname) return

    if (!wasmReady || !(window as any).demoparser2?.parseDemoBuffer) {
      alert('Парсер ещё загружается, подождите пару секунд и попробуйте снова.')
      return
    }
    ...
    try {
      ...
      const events = await (window as any).demoparser2.parseDemoBuffer(buffer)
  ...
  return (
    <>
      <Script
        ...
        onLoad={() => {
          // Даём время на инициализацию WASM
          setTimeout(() => {
            if ((window as any).demoparser2?.parseDemoBuffer) {
              setWasmReady(true)
              console.log('WASM parser ready')
            } else {
              console.error('demoparser2.parseDemoBuffer не найдена')
            }
          }, 2000)
        }}
      />
